    ...
    <div class="audit-header-tip">
      <div class="tip-content">
        ...
        <div class="text-group">
          ...
          <span class="desc"
            >为保障平台求职者权益，新发布或重大修改的岗位需经合规审核。审核通过后将自动转入「在招职位列表」并公开曝光。</span
          >
        </div>
      </div>
    </div>
      ...
      <el-tab-pane name="submitting">
        <template #label>
          <div class="tab-label-item">
            <span>审核中</span>
            <el-badge
              :value="auditList.filter((i) => i.auditStatus === 0).length"
              type="primary"
              class="badge-dot"
            />
          </div>
          ...
          <el-table-column label="操作" width="150" fixed="right">
            <template #default="scope">
              <el-button size="small" link type="primary" @click="handleView(scope.row)"
                >查看详情</el-button
              >
              <el-button size="small" link type="info" @click="handleWithdraw(scope.row)"
                >撤回</el-button
              >
            </template>
          </el-table-column>
      ...
      <el-tab-pane name="rejected">
        <template #label>
          <div class="tab-label-item">
            <span>未通过</span>
            <el-badge
              :value="auditList.filter((i) => i.auditStatus === 2).length"
              type="danger"
              class="badge-dot"
            />
          </div>
        ...
        <el-table :data="filterList(2)" style="width: 100%" v-loading="loading">
          ...
          <el-table-column label="操作" width="150" fixed="right">
            <template #default="scope">
              <el-button size="small" link type="danger" @click="handleReEdit(scope.row)"
                >重新编辑</el-button
              >
              <el-button size="small" link type="info" @click="handleDelete(scope.row)"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      <el-tab-pane name="drafts">
        <template #label>
          <div class="tab-label-item">
            <span>草稿箱</span>
            <el-badge
              :value="auditList.filter((i) => i.auditStatus === 3).length"
              type="info"
              class="badge-dot"
            />
          </div>
              ...
              <div class="position-cell">
                ...
                <span class="p-desc">{{
                  scope.row.requirement
                    ? scope.row.requirement.substring(0, 30) + '...'
                    : '暂无任职描述'
                }}</span>
              </div>
            </template>
          ...
          <el-table-column label="操作" width="150" fixed="right">
            <template #default="scope">
              <el-button
                ...
                type="primary"
                :icon="Edit"
                @click="handleResumeDraft(scope.row)"
                >继续编辑</el-button
              >
...
const handleWithdraw = (row: any) => {
  ElMessageBox.confirm(
    `确定要撤回职位【${row.title}】的发布申请吗？撤回后它将退回到草稿箱。`,
    '系统提示',
    {
