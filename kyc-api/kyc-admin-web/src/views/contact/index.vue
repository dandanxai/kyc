  ...
  <div class="chat-container">
    ...
    <div class="chat-right-panel">
      <div class="panel-section user-profile-center">
        ...
        <p class="p-sub"
          >{{ activeUser.gender }} ｜ {{ activeUser.age }}岁 ｜ {{ activeUser.city }}</p
        >
        <div class="p-tags">
      ...
      <div class="panel-section profile-details">
        ...
        <div class="detail-item">
          ...
          <span class="val"
            >{{ activeUser.school || '安徽大学' }} ({{ activeUser.edu || '本科' }})</span
          >
        </div>
        <div class="detail-item">
      ...
      <div class="panel-section quick-actions">
        ...
        <div class="action-buttons-grid">
          <el-button type="success" :icon="Check" plain @click="executeQuickAction('advance')"
            >直接晋级</el-button
          >
          <el-button type="danger" :icon="Close" plain @click="executeQuickAction('reject')"
            >不合适</el-button
          >
        </div>
  ...
  9001: [
    {
      sender: 'candidate',
      type: 'text',
      content: '您好，我已经关注贵公司很久了，对 Java 全栈及跨端混合开发岗位很感兴趣！',
      time: '14:20'
    },
    {
      sender: 'hr',
      type: 'text',
      content:
        '同学你好！看到你获得了中国软件杯和金砖国家技能大赛的一等奖，底子非常棒。我们项目刚好有很多跨端和本地 AI 集成需求。',
      time: '14:25'
    },
    {
      sender: 'hr',
      ...
      cardData: {
        ...
        time: '明天下午 15:30',
        type: '线上视频面试 (腾讯会议)',
        address: 'https://meeting.tencent.com/dm/xxxx'
      }
    },
  ...
  9002: [
    {
      sender: 'candidate',
      type: 'text',
      content: '您好，我精通 Vue3、ECharts 大屏开发，可以看看我的作品集。',
      time: '11:10'
    },
    {
      sender: 'hr',
      type: 'text',
      content: '收到，作品集很不错，等下帮您递交到业务技术主管那边交叉评估。',
      time: '11:12'
    },
    {
      sender: 'candidate',
      type: 'text',
      content: '请问咱们公司前端团队目前有多少人呢？',
      time: '11:15'
    }
  ],
  9003: [
    {
      ...
      type: 'text',
      content:
        '张先生您好，我们合肥研发中心目前在大规模扩充高并发中间件方向的核心骨干，看了您的中科大背景非常匹配。',
      time: '昨天 16:00'
    },
    {
      sender: 'candidate',
      type: 'text',
      content: '薪资架构里包含年终绩效和项目奖金吗？',
      time: '昨天 16:45'
    }
  ]
})
...
const filteredContacts = computed(() => {
  return contacts.value.filter(
    (c) => c.name.includes(searchUser.value) || c.targetJob.includes(searchUser.value)
  )
})
...
const sendQuickPhrase = () => {
  inputText.value =
    '同学你好，你的简历和背景我们团队非常看好，请问这周内什么时间方便约一轮线上的初试技术面对聊？'
}
...
const executeQuickAction = (action: 'advance' | 'reject') => {
  if (action === 'advance') {
    ElMessage.success(
      `快捷流转：候选人【${activeUser.value.name}】已被直接越级推送至下一轮面试池！`
    )
  } else {
    ElMessageBox.confirm(
      `确定要在聊天中将候选人【${activeUser.value.name}】做不合适挂起吗？`,
      '淘汰挂起',
      {
        type: 'warning'
      }
...
.msg-left .message-body {
  align-items: flex-start;
}
.msg-right .message-body {
