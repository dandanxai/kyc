      ...
      <div class="filter-flex-box">
        ...
        <div class="summary-right">
          <span class="summary-text"
            >当前多租户活跃漏斗人才：<strong>{{
              talentPipelines.reduce((acc, col) => acc + col.list.length, 0)
            }}</strong>
            人</span
          >
        </div>
...
const handleFilter = () => {
  ...
  talentPipelines.value = base.map((column: any) => {
    column.list = column.list.filter((c: any) => {
      const matchKey =
        c.name.includes(searchKeyword.value) || c.targetJob.includes(searchKeyword.value)
      let matchJob = true
...
const handleTalentAction = (
  ...
  if (action === 'advance' && typeof currentColIdx === 'number') {
    ...
    ElMessageBox.confirm(
      `确定要终止候选人【${candidate.name}】的本次岗位投递流程并归入淘汰库吗？系统将自动下发多租户通用拒绝信。`,
      '淘汰确认',
      {
    ...
    ElMessage.success(
      `🎉 正在为【${candidate.name}】生成由极客互娱签发的标准 SaaS 电子录用 Offer 报到通知...`
    )
  }
}
</script>
...
.col-screen .status-dot {
  background-color: #eab308;
}
.col-interview .status-dot {
  background-color: #10b981;
}
.col-offer .status-dot {
...
/* 美化自制滚动条组件 */
.column-card-scroller::-webkit-scrollbar {
  width: 4px;
}
.column-card-scroller::-webkit-scrollbar-thumb {
  background: #cbd5e1;
  border-radius: 4px;
}
.column-card-scroller::-webkit-scrollbar-track {
  background: transparent;
}
</style>
