    ...
    <el-card class="archive-table-card" shadow="never">
      ...
      <div class="archive-toolbar">
        <div class="toolbar-left">
          <el-button type="primary" plain :icon="Download" :disabled="selectedIds.length === 0"
            >批量导出简历</el-button
          >
          <el-button
            type="warning"
            ...
            <div class="talent-profile-cell">
              ...
              <div class="profile-meta">
                <div class="name-line">
                  ...
                  <el-tag size="small" type="success" effect="light" class="match-badge"
                    >AI 评分 {{ scope.row.aiScore }}</el-tag
                  >
                </div>
                <span class="t-sub"
                  >{{ scope.row.gender }} ｜ {{ scope.row.age }}岁 ｜ {{ scope.row.city }}</span
                >
              </div>
            </div>
          </template>
      ...
      <div class="archive-pagination">
        <el-pagination background layout="total, prev, pager, next" :total="1248" :page-size="10" />
      </div>
    </el-card>
  </div>
</template>
...
const handleActivate = (row: any) => {
  ElMessageBox.confirm(
    `确定要直接重置并激活候选人【${row.name}】的投递流程吗？激活后他将直接重返「新投递简历」首位。`,
    '流程激活提示',
    {
...
const handleRemove = (row: any) => {
  ElMessageBox.confirm(
    `警告：该操作将把【${row.name}】彻底从极客互娱的企业内部库移除。`,
    '移除资产确认',
    {
