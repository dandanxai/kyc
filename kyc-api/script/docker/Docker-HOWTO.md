...
依赖：docker compose v2，删除`name: kyc-system`，降低`version`版本为`3.3`以下，支持`docker-compose`。
...
```shell
# 创建maven缓存volume
docker volume create --name kyc-maven-repo

docker run -it --rm --name kyc-maven \
    -v kyc-maven-repo:/root/.m2 \
